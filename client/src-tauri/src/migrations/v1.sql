...
CREATE TABLE IF NOT EXISTS MESSAGE_ASSETS (
  ...
  -- this is used as a discriminant
  data BLOB NOT NULL,
  FOREIGN KEY(msg_id) REFERENCES MESSAGES(id) ON DELETE CASCADE
);
CREATE INDEX IF NOT EXISTS INDEX_MSG_CHAT_MAP ON MESSAGES(chat_id, created_at);
CREATE INDEX IF NOT EXISTS INDEX_ASSET_MSG_MAP ON MESSAGE_ASSETS(msg_id, role);
...
CREATE TRIGGER messages_au_timestamp BEFORE UPDATE ON MESSAGES 
BEGIN 
